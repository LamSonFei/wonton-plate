...
/**
 * Base mixin for the application.
 * Enables a more concise way of writing components.
 * Provides support for adding/removing events on connection/disconnection.
 */
export function WontonMixin(clazz) {
  return class extends clazz {
    ...
    constructor(props = {}) {
      ...
      if (typeof this.constructor.componentName === "function") {
        this._componentName = this.constructor.componentName() || `wtn-cmp-${Date.now()}`;
      } else {
        this._componentName = `wtn-cmp-${Date.now()}`;
      }
    ...
    connectedCallback() {
      ...
      this.classList.add(this._componentName);
      if (!this.useShadowDOM()) {
        // View template init
        this._injectTemplate();
        // References init
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      log.debug(
        `Updated ${name} from ${oldValue} to ${newValue} on ${
          this._componentName
        }!`
      );
    }
    ...
    _injectTemplate() {
      const html = typeof this.template === "function" ? this.template(this._props) : null;
      if (this.useShadowDOM()) {
        this.attachShadow({ mode: "open" });
        if (html) this.shadowRoot.innerHTML = html;
      } else if (html) {
        this.innerHTML = html;
      }
    ...
    _initReferences() {
      ...
      if (typeof this.references === "function") {
        ...
        (Object.getOwnPropertyNames(references) || []).forEach(
          refName =>
            (this._refs[refName] = (this.useShadowDOM()
              ? this.shadowRoot
              : this
            ).querySelector(references[refName]))
        );
      }
    }
      ...
      Object.getOwnPropertyNames(this._listeners).forEach(compRef => {
        ...
        Object.getOwnPropertyNames(compListeners).forEach(event => {
          (compRef !== "this"
            ? this.getRef(compRef)
            : this
          ).removeEventListener(event, compListeners[event]);
        });
      });
    }
  };
